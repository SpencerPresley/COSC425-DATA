...
def find_similar_titles(
    set1: Set[str], set2: Set[str], threshold: float = 0.8
) -> Set[str]:
    similar_titles = set()
    for title1 in set1:
        for title2 in set2:
...
def second_pass(dict_to_write: Dict) -> Dict:
    ...
    combined_set = set(dict_to_write["processed_categories_titles"]).union(
        new_combined_titles
    )
    ...
    return {
        "processed_categories_titles": dict_to_write["processed_categories_titles"],
        "titles_only_in_processed": set1,
        "titles_only_in_existing": set2,
        "combined_titles": list(new_combined_titles),
        "combined_titles_num": len(list(new_combined_titles)),
    }
...
def main():
    ...
    dict_to_write = {}
    dict_to_write["processed_categories_titles"] = list(
        titles_from_processed_category_data
    )

    print(
        "Getting titles from existingTitles_missingCitations_missingArticlesTitles.json..."
    )
    titles_from_existing_titles_missing_citations_missing_articles_titles = get_titles_from_json_file(
        "assets/json_data/output_verification_using_dm/existingTitles_missingCitations_missingArticlesTitles.json"
    )

    combined_titles = titles_from_processed_category_data.union(
        titles_from_existing_titles_missing_citations_missing_articles_titles
    )
    ...
    similar_titles_in_processed = find_similar_titles(
        titles_from_processed_category_data,
        titles_from_existing_titles_missing_citations_missing_articles_titles,
    )
    similar_titles_in_existing = find_similar_titles(
        titles_from_existing_titles_missing_citations_missing_articles_titles,
        titles_from_processed_category_data,
    )

    print("Calculating titles only in processed category data...")
    titles_only_in_processed = (
        titles_from_processed_category_data - similar_titles_in_existing
    )

    print(
        "Calculating titles only in existing titles missing citations missing articles titles..."
    )
    titles_only_in_existing = (
        titles_from_existing_titles_missing_citations_missing_articles_titles
        - similar_titles_in_processed
    )

    print("Titles only in processed category data:", titles_only_in_processed)
    print(
        "Titles only in existing titles missing citations missing articles titles:",
        titles_only_in_existing,
    )
    ...
    with open(
        "assets/json_data/output_verification_using_dm/titles_comparison.json", "w"
    ) as file:
