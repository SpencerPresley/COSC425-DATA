...
def get_conda_package(pip_package):
    ...
    result = subprocess.run(['conda', 'search', package_name, '--json'], capture_output=True, text=True)
...
def main():
    ...
    with open('install_packages.sh', 'w') as f:
        f.write("#!/bin/bash\n\n")
        f.write("# Activate conda environment (uncomment and replace with your environment name)\n")
