...
class ClassificationWrapper:
    ...
    def __init__(
        ...
        self.unclassified_details_dict = {
            "dois": [],
            "abstracts": [],
            "items": []
        }
    ...
    def _classification_orchestrator(self, data: List[Dict]):
        ...
        for item in data:
            ...
            self._inject_categories(
                data=item,
                categories=self._extract_categories(doi, classifier)
            )
    ...
    def _retrieve_doi_abstract(
        ...
        result = self.utilities.get_attributes(
            item, 
            [AttributeTypes.CROSSREF_DOI, AttributeTypes.CROSSREF_ABSTRACT]
        )
        doi = result[AttributeTypes.CROSSREF_DOI][1] if result[AttributeTypes.CROSSREF_DOI][0] else None
        abstract = result[AttributeTypes.CROSSREF_ABSTRACT][1] if result[AttributeTypes.CROSSREF_ABSTRACT][0] else None
        return doi, abstract
    ...
    def _update_classified_instance_variables(
        ...
        self.unclassified_dois.append(doi) if doi else self.unclassified_dois.append("NULL")
        self.unclassified_abstracts.append(abstract) if abstract else self.unclassified_abstracts.append("NULL")
        self.unclassified_doi_abstract_dict[doi] = abstract
        self.unclassified_items.append(item)
        self.unclassified_details_dict["dois"].append(doi) if doi else self.unclassified_details_dict["dois"].append("NULL")
        self.unclassified_details_dict["abstracts"].append(abstract) if abstract else self.unclassified_details_dict["abstracts"].append("NULL")
