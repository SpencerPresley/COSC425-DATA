...
class APIKeyValidator:
    ...
    def _validate(self, api_key: str, model: Optional[str] = None) -> None:
        ...
        try:
            ...
            llm = ChatGoogleGenerativeAI(api_key=api_key, model=model or "gemini-1.5-pro")
            chain = prompt | llm
            chain.invoke({})
            results.google = True
        except Exception:
    ...
    def print_results_for_api_key(self, api_key: str) -> None:
        ...
        for service, valid in results.items():
            status = f"{unicode_chars_dict.get('boxed_checkmark', '')} Valid" if valid else f"{unicode_chars_dict.get('boxed_x', '')} Invalid"
