...
salisbury_faculty = list(salisbury_faculty)  # Should be ['KwangWook Gang', 'Minseok Park', 'Christy H. Weer']

# Create collaboration matrix for Salisbury faculty
collaboration_matrix = pd.DataFrame(0, 
                                  index=salisbury_faculty, 
                                  columns=salisbury_faculty)
...
affiliation_citation_matrix = pd.DataFrame(0, 
                                         index=affiliations, 
                                         columns=['article_count', 'total_citations'])
...
sns.heatmap(affiliation_citation_matrix, 
            annot=True, 
            fmt='.0f', 
            cmap='YlOrRd',
