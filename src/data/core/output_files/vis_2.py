    ...
    for faculty, stats in category.get('faculty_stats', {}).items():
        ...
        for paper_title in stats['citation_map']['article_citation_map'].keys():
            for other_faculty, other_stats in category.get('faculty_stats', {}).items():
                if faculty != other_faculty and paper_title in other_stats['citation_map']['article_citation_map']:
                    collaborations[(min(faculty, other_faculty), max(faculty, other_faculty))] += 1
...
for faculty in faculty_citations:
    G.add_node(faculty, citations=faculty_citations[faculty], dept=faculty_dept[faculty])

# Draw network
pos = nx.spring_layout(G, k=3, iterations=50)  # Increased k for more spread
node_sizes = [max(200, G.nodes[node]['citations'] * 10) for node in G.nodes()]  # Minimum size of 200
edge_weights = [max(1, G.edges[edge]['weight']) for edge in G.edges()]  # Minimum weight of 1

# Color nodes by department
dept_colors = {dept: plt.cm.tab20(i) for i, dept in enumerate(set(faculty_dept.values()))}
node_colors = [dept_colors[G.nodes[node]['dept']] for node in G.nodes()]

# Create legend for departments
legend_elements = [plt.Line2D([0], [0], marker='o', color='w', 
                            markerfacecolor=color, label=dept, markersize=8)
...
dept_metrics = pd.DataFrame({
    ...
    'Citations per Paper': {k: round(v['citations']/v['papers'], 2) if v['papers'] > 0 else 0 
...
sns.heatmap(dept_metrics_normalized.sort_values('Total Citations', ascending=False).head(15),
            cmap='RdYlBu',
            center=0,
            annot=dept_metrics.sort_values('Total Citations', ascending=False).head(15),  # Show actual values
            fmt='.0f')  # No decimal places for actual values

plt.title('Department Research Impact Comparison\n\n' +
...
plt.title('Distribution of Faculty Citations\n\n' +
