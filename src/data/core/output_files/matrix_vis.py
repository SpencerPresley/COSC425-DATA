...
def create_line_chart(categories, level_name):
    ...
    plt.plot(categories, [data[cat]['tc_count'] for cat in categories], 'o-', linewidth=2, markersize=10, label='Total Citations')
    plt.plot(categories, [data[cat]['citation_average'] for cat in categories], 's-', linewidth=2, markersize=10, label='Citation Average')
...
# Create all charts
for categories, level in [(top_level_categories, 'Top'), 
