...
import requests
from academic_metrics.main import WosClassification
...
def query_crossref(title: str, max_retries: int = 3, delay: int = 2) -> Tuple[Dict[str, Any], float]:
    # Convert the title to lowercase except for the first letter of the first word
    words = title.split()
    if words:
        title = words[0].capitalize() + ' ' + ' '.join(word.lower() for word in words[1:])
        ...
        if response.status_code == 200:
            ...
            if 'message' in data and 'items' in data['message']:
                ...
                if items:
                    ...
                    # input("Press Enter to continue...")
                    return json.dumps(items[0], indent=4), time.time() - start_time  # Return the first item and query time
            return {}, time.time() - start_time  # Return an empty dict and query time if no items are found
        else:
...
def load_titles_from_reformatted_files(directory_path: str) -> Set[str]:
    ...
    for file_path in tqdm(json_files, desc="Loading titles from reformatted files", position=0, leave=True):
...
def load_titles_from_processed_category_data(file_path: str) -> Set[str]:
    ...
    with open(file_path, 'r') as f:
        data: Dict[str, Any] = json.load(f)
    titles_set: Set[str] = set()
    for category, info in tqdm(data.items(), desc="Loading titles from processed category data", position=0, leave=True):
...
def process_titles(titles: Set[str]) -> Set[str]:
    ...
    """
    return set(aggressive_regularize(title) for title in tqdm(titles, desc="Processing titles", position=0, leave=True))

def compare_datasets(dm_titles: Set[str], wos_titles: Set[str]) -> Tuple[Set[str], Set[str], Set[str]]:    
    """
...
def find_similar_titles(dm_titles: Set[str], wos_titles: Set[str], threshold: float = 0.8, crossref:bool=False) -> Set[str]:
    """
    ...
    for idx, title in enumerate(tqdm(wos_titles_list, desc="Indexing WoS titles", position=0, leave=True)):
    ...
    for dm_title in tqdm(dm_titles, desc="Finding similar titles", position=0, leave=True):
        m: MinHash = MinHash(num_perm=128)
...
def display_titles_only_in_dm(final_only_in_dm: Set[str], dm_titles_set: Set[str], wos_or_crossref_str: str) -> None:
    """
    ...
    for title in tqdm(dm_titles_set, desc="Displaying titles only in DM", position=0, leave=True):
...
def print_fancy_box(message: str) -> None:
    ...
    for line in lines:
        ...
        print("║" + " " * padding + line + " " * (box_width - len(line) - padding) + "║")

    print("║" + " " * box_width + "║")  # Add empty line for bottom padding
    print("╚" + "═" * box_width + "╝\n")
    
    with open(os.path.join(os.path.dirname(__file__), 'static_output_files', 'REAL_verification_results_SUMMARY.txt'), 'w') as f:
        print("\n╔" + "═" * box_width + "╗", file=f)
        print("║" + " " * box_width + "║", file=f)  # Add empty line for top padding

        for line in lines:
            padding: int = (box_width - len(line)) // 2
            print("║" + " " * padding + line + " " * (box_width - len(line) - padding) + "║", file=f)
...
def main(crossref:bool=False, second_run:bool=False) -> None:
    ...
    reformatted_files_dir: str = os.path.join(os.path.dirname(__file__), 'reformattedFiles')
    processed_category_data_file: str = os.path.join(os.path.dirname(__file__), 'static_output_files', 'processed_category_data.json')
    test_processed_category_data_file: str = os.path.join(os.path.dirname(__file__), 'test_processed_category_data.json')
    
    wos_or_crossref_str: str = 'Web of Science Export' if not crossref else 'Crossref API'
    
    dm_titles_set: Set[str] = load_titles_from_reformatted_files(reformatted_files_dir)
    wos_titles_set: Set[str] = set()
    if crossref:
        wos_titles_set: Set[str] = load_titles_from_processed_category_data(test_processed_category_data_file)
    else:
        wos_titles_set: Set[str] = load_titles_from_processed_category_data(processed_category_data_file)

    print(f"Initial Digital Measures titles: {len(dm_titles_set)}")
    print(f"Initial {wos_or_crossref_str} titles: {len(wos_titles_set)}")

    exact_matches, only_in_dm, only_in_wos = compare_datasets(dm_titles_set, wos_titles_set)
    
    print("\nInitial comparison:")
    print(f"Exact matches: {len(exact_matches)}")
    print(f"Only in Digital Measures: {len(only_in_dm)} = {len(dm_titles_set)} - {len(exact_matches)}")
    print(f"Only in {wos_or_crossref_str}: {len(only_in_wos)} = {len(wos_titles_set)} - {len(exact_matches)}")
    ...
    similar_titles: Set[str] = find_similar_titles(only_in_dm, only_in_wos, threshold=0.8, crossref=crossref)
    print(f"Similar titles found: {len(similar_titles)}")
        
    final_common_set: Set[str] = exact_matches.union(set(aggressive_regularize(title) for title in similar_titles))
    final_only_in_dm: Set[str] = only_in_dm - set(aggressive_regularize(title) for title in similar_titles)
    final_only_in_wos: Set[str] = only_in_wos - set(aggressive_regularize(title) for title in similar_titles)

    print("\nFinal results:")
    print(f"Final common set: {len(final_common_set)} = {len(exact_matches)} + {len(similar_titles)}")
    print(f"Final only in Digital Measures: {len(final_only_in_dm)} = {len(only_in_dm)} - {len(similar_titles)}")
    print(f"Final only in {wos_or_crossref_str}: {len(final_only_in_wos)} = {len(only_in_wos)} - {len(similar_titles)}")

    total_unique: int = len(final_common_set) + len(final_only_in_dm) + len(final_only_in_wos)
    ...
    wos_exclusive_percentage: float = (len(final_only_in_wos) / len(wos_titles_set)) * 100
    ...
    print(f"Percentage of {wos_or_crossref_str} titles not in Digital Measures: {wos_exclusive_percentage:.2f}% = ({len(final_only_in_wos)} / {len(wos_titles_set)}) * 100")
    
    
    percent_wos_has_from_dm: float = 100 - ((len(final_only_in_dm) / len(dm_titles_set)) * 100)
    print(f"Percent of Digital Measures titles that {wos_or_crossref_str} has: {percent_wos_has_from_dm:.2f}%")
    ...
    if crossref and len(final_only_in_dm) > 0:
        ...
        dm_titles_set = {title for title in dm_titles_set if aggressive_regularize(title) != aggressive_regularize(title_to_exclude)}
        final_only_in_dm = {title for title in final_only_in_dm if aggressive_regularize(title) != aggressive_regularize(title_to_exclude)}

        # Inform the user about the exclusion
        print(f"The paper '{title_to_exclude}' is published in the Tax Adviser, which does not provide a DOI. It is considered a professional magazine/trade publication rather than an academic journal.")

        with tqdm(total=len(final_only_in_dm), desc="Querying Crossref API", position=0, leave=True) as pbar:
            if not second_run:
                if len(final_only_in_dm) > 0:
                    for i, title in enumerate(final_only_in_dm):
                        ...
                        original_title = next((t for t in dm_titles_set if aggressive_regularize(t) == title), None)
                        if original_title:
                        ...
                        if i == 0:
                            estimated_time_per_query = query_time * (len(final_only_in_dm) - i + 1)
                        
                        pbar.update(1)
                        if estimated_time_per_query:
                            pbar.set_postfix({"Estimated time left": f"{(len(final_only_in_dm) - pbar.n) * estimated_time_per_query:.0f}s"})
                
                    # Save crossref results to a file in 'input_files' directory via appending to the current file there
                    crossref_file_path = os.path.join(os.path.dirname(__file__), 'input_files', '2017-2024-paper-doi-list.json')
                    ...
                    print(f"second_run: {second_run}, crossref_results: {len(crossref_results)}")
                    input("Press Enter to continue...")
