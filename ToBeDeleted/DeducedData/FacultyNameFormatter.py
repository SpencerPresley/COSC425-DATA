...
class FacultyNameFormatter:
    def __init__(self):
        self.category_dict: dict[str, CategoryInfo] = (
            PickleDictLoader.load_pickle_dict()
        )
        self.format_pattern = re.compile(r"^[A-Za-z]+,\s[A-Za-z]+\s[A-Z]\.$")
        self.pseudo_unique_fac_set = ConstructPseudoUniqueFaculty(
            self.category_dict
        ).get_pseudo_unique_faculty_set()
        self.refined_and_unique_fac_set: set[str] = set()
        self.construct_formatted_faculty_name_set(
            self.refined_and_unique_fac_set,
            self.pseudo_unique_fac_set,
            self.format_pattern,
        )
    ...
    @staticmethod
    def construct_formatted_faculty_name_set(
        refined_and_unique_fac_set: set[str],
        pseudo_unique_faculty_set: set[str],
        format_pattern: Pattern[str],
    ) -> set[str]:
