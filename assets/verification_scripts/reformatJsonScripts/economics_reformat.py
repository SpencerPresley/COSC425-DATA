...
class TitleExtractor:
    ...
    def extract_title_from_citation(self, citation):
        ...
        # Remove URLs
        citation = re.sub(r'http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\\(\\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+', '', citation)
...
accounting_json_path = os.path.abspath(
    os.path.join(current_file_location, '..', '..', 'json_data', 'Accounting-and-Legal-Studies.json')
)
...
ids_json_path = os.path.abspath(
    os.path.join(current_file_location, '..', '..', 'json_data', 'Information-and-Decision-Sciences.json')
)
